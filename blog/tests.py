...
class PostTest(TestCase):
    def setUp(self):
        # 본 테스트에 공통으로 사용하는 데이터
        # Test Users 를 만들고 이 이용자가 접근한다는 전제로 사용함
        self.client = Client()

    def _add_post_by_http(self, **kwargs):
        follow = kwargs.pop('follow', True)
        kwargs['post_writer'] = 'donald'

        # 글 게시 시도
        return self.client.post(
            '/blog/update/', kwargs, follow=follow,
        )
    ...
    def test_list_post(self):
        ...
        for i in range(expected_count):
            response = self._add_post_by_http(
                post_category=category.pk,
                post_title='hello world {}'.format(i),
                post_content='Lorem Ipsum'
            )
            self.assertEqual(response.status_code, 200)
        ...
        # 글 목록 페이지 접근
        response = self.client.get('/blog/list/', follow=True)
        # http status code가 200인지 확인
        self.assertEqual(response.status_code, 200)
        # 이동한 URL의 뷰 함수가 list_post 인지 테스트
        self.assertEqual(response.resolver_match.func, views.list_posts)
        # 템플릿 컨텍스트에 posts (page_info) 가 있는지 확인
        self.assertIn('page_info', response.context)
        # 템플릿 변수 posts의 데이터 개수 가져오기
        res_count = len(response.context['page_info']['post_in_page'])
    ...
    def test_view_post(self):
        ...
        # 존재하지 않는 포스트에 접근
        response = self.client.get('/blog/list/1/', follow=True)
